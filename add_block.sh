...
# Create safe slug for filenames: lowercase, underscores, no slashes
block_slug=$(echo "$block_name" | tr '[:upper:]' '[:lower:]' | tr ' ' '_' | sed 's#[\\/ ]##g' | sed 's/[^a-z0-9_]/_/g')

# Create kebab-case version for ACF block name: lowercase, dashes, no slashes
block_kebab=$(echo "$block_name" | tr '[:upper:]' '[:lower:]' | tr ' ' '-' | sed 's#[\\/ ]##g' | sed 's/[^a-z0-9-]/-/g')
